...
useEffect(() => {
  const fetchProperties = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://backend-lomi.onrender.com/api/properties/allproperties");
      if (!response.ok) {
  ...
  const handleDeleteProperty = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this property?')) {
      try {
        const response = await fetch(`https://backend-lomi.onrender.com/api/properties/${id}`, {
          method: "DELETE",
        });
        if (!response.ok) {
