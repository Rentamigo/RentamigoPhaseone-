...
//     try {
//       const response = await axios.get("https://backend-lomi.onrender.com/api/employees");
...
//     try {
//       await axios.put(`https://backend-lomi.onrender.com/api/employees/${id}`, data);
  ...
  {users.length === 0 ? (
    ...
    users.map((user, index) => (
      <tr key={user.id} className="border-b">
        ...
        <td className="p-4">{index + 1}</td> {/* Use index for serial number */}
        <td className="p-4">{user.name}</td>
        <td className="p-4">{user.email}</td>
        <td className="p-4">{user.mobile}</td>
        <td className="p-4">{user.access}</td>
